...
- 学校 4093 官方环境完整 43 项测试通过，包含新增全局 cap、筛选参数、
  固定 token 名单恢复、旧 NPZ 复用与带小数点 log 名称的处理。
- 两个真实 mini DB 设置全局 cap=6：合计选中6个场景并产出6个有效NPZ，
  不是每DB各6个；恢复时特征 hash/mtime 不变。
- 新目录重选相同名单后，6个NPZ全部通过校验并原地复用；随后使用官方发布
  checkpoint 构建6条场景特征缓存成功。测试输出均置于独立临时目录。
- 学校连接恢复后完成上述实际接口测试；未在公司服务器上执行100万样本任务，
  不将小规模接口测试耗时外推为全量运行时间。

## 2026-09-24 八组训练入口验证

- 4093 的现有官方环境中，35 项测试全部通过，包含八组配置/阶段调度、
  单组失败汇总、全局首尾依赖、邻车 padding、邻车缓存绑定及精确断点恢复。
- 使用官方模型类与临时合成 checkpoint/NPZ，执行共享缓存、预测邻车缓存、
  训练、诊断和 planner refinement；邻车缓存重复运行不会重写已完成 shard。
- 八组完整 width192 配置逐一在空闲 RTX4090D 上通过 microbatch128 CUDA 前向/
  反向检查，梯度有限；L/W 为 1,387,204 参数，G 为 1,832,068，N 为 1,634,692。
  单次分支检查 allocated 峰值约 0.40/0.59/0.45 GiB，不含完整训练优化器或 planner 缓存。
- 启动器 Bash 语法、无副作用 dry-run、tmux 启动及缺失数据时的明确退出已验证。
  已安装官方环境的依赖解析检查通过；固定 pip23.3.2 兼容旧 OmegaConf 依赖元数据。
- 公司 8×A100 的完整安装/缓存/训练尚未实机执行；学校服务器上的 GPU 检查为单卡依次执行。
  上述验收为工程正确性检查，未产生这八组真实训练或规划性能结论。
